...
默认保存到 `received/<本次运行目录>/transfer-*/`，可用 `--receive-dir` 指定其他目录。同一运行目录内的 `events.jsonl` 记录任务和收件结果；终端也会显示主要事件。按 Ctrl+C 停止。可与 `video` 在两个终端同时运行。
...
已通过本机模拟发送测试，以及**原厂组件在回环地址发送 → Python 接收**的兼容性测试，涵盖目录、中文文件名和约 1 MiB 的二进制文件。真实课堂观察确认：教师端只向已登录的学生分配文件；原接收器连接 8555 后先发送节点心跳 6，再发送接收端就绪 7，收到接收任务 3 后，教师机主动连接学生的 9100 发送内容。本命令已经实现 6/7 和内容接收，并由同一进程建立 9003 学生登录连接；其他学生节点中继、自动切换端口和完成报告也尚未实现。任务中的发送节点地址可能是教师机内部地址，因此只作记录；本地任务地址和端口用于校验监听参数。
...
现场已确认 `--mock-thumbnail` 回传的固定测试图可在教师界面显示。目录查询应答也已加入：收到命令 87 的查询后，用命令 88 提供 `--receive-dir` 指定的收件目录（默认 `received`），不枚举磁盘或子目录。此目录应答尚待教师界面实测；文件监听随 `client` 自动启动，收件目录统一使用 `--receive-dir`；实际文件保存在本次运行子目录内。
...
该原型只记录管理命令，不执行截图、键鼠控制、启动程序等操作。可加 `--mock-thumbnail`，在收到 64×64 缩略图请求时回传随程序附带的固定 MOCK 测试图，用于检查教师界面显示；默认不回复缩略图。文件内容由同一进程的接收线程处理，管理报文同时持续处理。按 Ctrl+C 退出，管理连接断开后也会退出，同时停止文件任务和数据线程；当前不自动重连管理连接。不应同时运行官方学生端和本原型。
